...
- 결국 문제는 \\(P^-1AP\\) 를 대각행렬로 만드는 `좋은 행렬` \\(P\\)를 구하는 것
- 폭주의 판정  
...
- \\(A\\)와 같은 크기의 정칙행렬 \\(S\\)에 대해, \\[\left(S^-1AS\right)\left(S^-1p\right)=\lambda \left( S^-1p \right)\\] 이므로, \\(S^-1p\\)는 \\(S^-1AS\\)의 고유벡터며 고유값은 변함없이 \\(\lambda\\)임 : `닮음변환으로 고유값은 변하지 않는다`
