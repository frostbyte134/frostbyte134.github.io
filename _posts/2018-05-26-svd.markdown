...
> Singular value decomposition (SVD): Any \\(m\times n\\) matrix \\(A\\) can be decomposed into \\[A=U\Sigma V^T = (m\times n\textrm\{ orthogonal\})(m\times n\textrm\{ diagonal\})(n\times n\textrm\{ orthogonal\}).\\]
...
Let \\(A\in\mathbb\{C\}^\{m\times n\}\\). WLOG we see that \\(\textrm\{min\}(m,n)\leq r=\textrm\{rank\}A\\).  
