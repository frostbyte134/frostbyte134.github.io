...
- `SW아키텍처 설계` : 에서는 컴포넌트 수행 주기, 컴포넌트 수행 구조에 대한 대안 도출
- `(컴포넌트)상세 설계` : 에서는 내부 수행 순서, 알고리즘 자체 평가 수행
...
측정지표
- `성과 가치` (earned value): 프로젝트 리소스 및 비용. 완료된 태스크의 계획 공수 합
- `투입 공수` : __프로젝트 리소스 및 비용. 업무 진도 및 일정과는 상관없다고 함.__ 실화냐. 이거 월요일에 함 더 보자
...
- 개인관점은 미포함하도록. __문제점만 발견 (해결책까지 찾는다고 하면 틀림)__
