...
- `외부 결합도` : 특정 모듈에서 반환한 값을 다룬 모듈에서 참조해서 쓰는 경우
- `공통 결합도` : 전역변수를 이용한 결합
...
- `SW통합 테스트`: 아키텍처 설계서를 기준으로 테스트 수행. 함수/호출 커버리지를 측정해야 함
- `SW 테스트`: 요구사항 커버리지를 측정해야 함 (함수 커버리지가 아님)
- `SW 단위 테스트`: 상세설계서 기준으로 테스트 수행. 문장/분기/MDCD 커버리지를 측정해야 함
